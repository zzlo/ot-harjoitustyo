...
class Level:
    def __init__(self, display_size):
        self.text_surface = pygame.Surface((display_size, display_size), pygame.SRCALPHA)
        ...
        for y in range(height):
            for x in range(width):
                ...
                if self.game_state[y][x] != 0:
                    ...
                    self.text_surface.blit(text, (normalized_x + tile_width/2 - spacing/2, normalized_y + tile_height/2 - spacing))
    ...
    def _combine(self):
        for x in range(4):
            for y in range(3):
                if self.game_state[x][y] == 0: continue
